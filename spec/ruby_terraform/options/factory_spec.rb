require 'spec_helper'
 ...
 describe RubyTerraform::Options::Factory do
  ...
  let(:test_data) do
    {
      my_standard: {
        ...
        class: RubyTerraform::Options::Standard,
        option: standard
      },
      my_boolean: {
        switch: '-my-boolean',
        value: true,
        class: RubyTerraform::Options::Boolean,
        option: boolean
      },
      my_flag: {
        switch: '-my-flag',
        value: false,
        class: RubyTerraform::Options::Flag,
        option: flag
      },
      my_plural: {
        switch: '-my-plural',
  ...
  let(:standard) { instance_double(RubyTerraform::Options::Standard) }
  let(:flag) { instance_double(RubyTerraform::Options::Flag) }
  let(:boolean) { instance_double(RubyTerraform::Options::Boolean) }
  let(:singular) { instance_double(RubyTerraform::Options::Standard) }
  let(:override) { instance_double(RubyTerraform::Options::Standard) }
  let(:plural) { instance_double(RubyTerraform::Options::Standard) }

  before do
    stub_options(test_data)
    stub_const(
      'RubyTerraform::Options::Factory::PLURAL_SWITCHES',
      Set.new(['-my-plural'])
    )
    stub_const(
      'RubyTerraform::Options::Factory::BOOLEAN_SWITCHES',
      Set.new(['-my-boolean'])
    )
    stub_const(
      'RubyTerraform::Options::Factory::FLAG_SWITCHES',
      Set.new(['-my-flag'])
    )
    stub_const(
      'RubyTerraform::Options::Factory::OVERRIDE_SWITCHES',
      { override: :option_key }
    )
    options
  end
  ...
  describe '.from' do
    it 'creates a Standard Option with the singular switch and singular value for each plural options' do
      expect(RubyTerraform::Options::Standard).to have_received(:new).with(
    ...
    it 'creates a Boolean Option for each boolean switch' do
      expect(RubyTerraform::Options::Boolean).to have_received(:new).with(
        test_data[:my_boolean][:switch], test_data[:my_boolean][:value]
      )
    end

    it 'creates a Flag Option each flag switch' do
      expect(RubyTerraform::Options::Flag).to have_received(:new).with(
