...
describe RubyTerraform::Commands::Destroy do
  ...
  it 'correctly serialises list/tuple vars' do
    ...
    expect(Open4).to(
        receive(:spawn)
            .with("terraform destroy -var 'list=[1,\"two\",3]' some/configuration", any_args))
  ...
  it 'correctly serialises map/object vars' do
    ...
    expect(Open4).to(
        receive(:spawn)
            .with("terraform destroy -var 'map={\"first\":1,\"second\":\"two\"}' some/configuration", any_args))
  ...
  it 'correctly serialises vars with lists/tuples of maps/objects' do
    ...
    expect(Open4).to(
        receive(:spawn)
            .with("terraform destroy -var 'list_of_maps=[{\"key\":\"value\"},{\"key\":\"value\"}]' some/configuration", any_args))
  ...
  it 'adds a target option if a target is provided' do
    ...
    expect(Open4).to(
        receive(:spawn)
            .with("terraform destroy -target=some_resource_name some/configuration", any_args))
  ...
  it 'adds a target option for each element of target array' do
    ...
    expect(Open4).to(
        receive(:spawn)
            .with("terraform destroy -target=some_resource_1 -target=some_resource_2 some/configuration", any_args))
  ...
  it 'ensures that target and targets options work together' do
    ...
    expect(Open4).to(
        receive(:spawn)
            .with("terraform destroy -target=some_resource_1 -target=some_resource_2 -target=some_resource_3 some/configuration", any_args))
