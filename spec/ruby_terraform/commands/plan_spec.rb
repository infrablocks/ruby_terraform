...
describe RubyTerraform::Commands::Plan do
  ...
  it 'correctly serialises list/tuple vars' do
    ...
    expect(Open4).to(
        receive(:spawn)
            .with("terraform plan -var 'list=[1,\"two\",3]' some/configuration", any_args))
  ...
  it 'correctly serialises map/object vars' do
    ...
    expect(Open4).to(
        receive(:spawn)
            .with("terraform plan -var 'map={\"first\":1,\"second\":\"two\"}' some/configuration", any_args))
  ...
  it 'correctly serialises vars with lists/tuples of maps/objects' do
    ...
    expect(Open4).to(
        receive(:spawn)
            .with("terraform plan -var 'list_of_maps=[{\"key\":\"value\"},{\"key\":\"value\"}]' some/configuration", any_args))
